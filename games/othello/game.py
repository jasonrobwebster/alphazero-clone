...
class Othello(Game):
    ...
    def winner(self):
        ...
        if result == '*':
            # game still going
            return 0
        # TODO: Make other engines more intuitive by returning the winning player, not the losing players result
        if result == '1-0' or result == '0-1':
            # it's always the turn of the losing player
            # return the winning value for the opposing player
            return -1
        if result == '1/2-1/2':
            return 0.00001
    ...
    def get_symmetries(self, policy, nnet=False):
        syms, policies = self.engine.symmetries(policy)
        if nnet:
            syms = list(map(lambda x: self.represent_nn(x), syms))
        return syms, policies